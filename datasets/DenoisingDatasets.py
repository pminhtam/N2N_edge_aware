...
class BenchmarkTrain(BaseDataSetH5):
    def __init__(self, folder_path, length, pch_size=256):
    ...
    def __getitem__(self, index):
        ...
        mask_edge = self.create_mask(input_red, input_blue)
        ...
        mask_edge = torch.from_numpy(mask_edge.transpose((2, 0, 1)))
        return im_noisy, im_gt, input_red, input_blue, mask_red, mask_blue, mask_edge


    def randomize(self, H, W):
        mask = np.zeros((H, W))
        pattern_mask = np.random.randint(0, 8, (H//2, W//2))
        mask = np.zeros((H, W))
        for h in range(H // 2):
            for w in range(W // 2):
                h1 = h*2 ; h2 = h1 + 2
